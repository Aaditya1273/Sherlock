...
/**
 * Lightfall — a WebGL streak-field background (from the user-supplied cta.md).
 *
 * Renders falling light streaks on a deep blue glow with mouse-following
 * light. Used only inside the dark closing call to action. Client-only: it
 * touches the canvas and requestAnimationFrame, so it is mounted with
...
export default function Lightfall({
  ...
  // Stable identity for the colour list so a re-render does not rebuild the GL context.
  ...
  useEffect(() => {
    ...
    // Only render while on screen — a WebGL loop at the bottom of a long
    // page should not cost anything while the reader is at the top.
    let visible = true;
    ...
    dpr, paused, colorKey, backgroundColor, speed, streakCount, streakWidth, streakLength, glow,
    density, twinkle, zoom, backgroundGlow, opacity, mouseInteraction, mouseStrength, mouseRadius,
    mouseDampening, lightMode,
  ]);
